...
router.get('/', auth, async (req, res) => {
  try {
    // If user is not admin, only return their own contributions
...
router.get('/month/:month/year/:year', auth, async (req, res) => {
  try {
    ...
    const contributions = await Contribution.find({ month, year: parseInt(year) })
      .populate('user', 'name')
      .populate('approvedBy', 'name');
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (existingContribution) {
      return res.status(400).json({ message: 'Contribution already submitted for this month' });
    }
